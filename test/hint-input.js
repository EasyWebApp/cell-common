...
describe('hint-input', () => {
    ...
    it('Constructor', async () => {
        (await page.$eval(
            'hint-input',
            input => input.shadowRoot.nodeType
        )).should.be.equal(11);

        (await page.$eval('hint-input', input =>
            Array.from(input.shadowRoot.children, child =>
                child.tagName.toLowerCase()
            )
        )).should.be.eql(['style', 'label', 'input', 'datalist', 'slot']);
    });

    it('Observe attributes', () =>
        page
            .$eval('hint-input', input => [
                input.label,
                input.placeholder,
                input.value
            ])
            .should.be.fulfilledWith(['Selector', 'selector', '']));
    ...
    it('Attribute & Property', () =>
        page
            ...
            .should.be.fulfilledWith([
                'text',
                './hint-input.json',
                'json.districts.districts.name'
            ]));

    it('Slot options', async () => {
        (await page.$eval('hint-input', input =>
            Array.from(input.options, node => node.nodeType).filter(
                type => type === 1
            )
    ...
    it('Change event', async () => {
        ...
        await page.$eval(
            'hint-input',
            input =>
                (input.onchange = ({ target }) => console.info(target.tagName))
        );
        ...
        (await page.$eval('hint-input', input => input.value)).should.be.equal(
            'x'
        );
